...
def resolver_ruta(ruta_relativa):
    if getattr(sys, 'frozen', False):
        # Si es el .exe, busca en la carpeta temporal de PyInstaller
        return os.path.join(sys._MEIPASS, ruta_relativa)
    
    # SI ES EN VS CODE: Busca en la misma carpeta donde está ESTE archivo .py
    carpeta_actual = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(carpeta_actual, ruta_relativa)

# Ahora cargamos el archivo asegurando la ruta correcta
Builder.load_file(resolver_ruta("guardar.kv"))
...
Builder.load_file(resource_path("guardar.kv"))
